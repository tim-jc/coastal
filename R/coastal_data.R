...
load_coastal_data <- function(
  ...
  validate_coastal_activities(activities, connection = connection, check_db = validate_db)
  validate_ferries(ferries_data, activities)

  full_dataset <- load_gps_data(connection, activities)
  rides_index <- create_summary(full_dataset, connection, activities$activity_id)
  rider_ids <- extract_riders(activities)
  image_metadata <- if (include_images) get_image_metadata() else NULL
  position_extremities <- if (include_position_extremities) get_position_extremities(full_dataset) else NULL
