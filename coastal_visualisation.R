...
full_dataset <- coastal_data$coastal_streams
...
for (rider in riders) {
  ...
  summary_data <- rider_index %>%
    ...
    group_by(yr) %>%
    summarise(
      total_miles = sum(coastal_distance_miles),
      total_climb = sum(coastal_elevation_metres)
    ) %>%
  ...
  miles <- summary_data %>%
    ...
    geom_text(
      data = lbl_df,
      aes(x = yr, y = total_miles, label = miles_lbl),
      colour = "#FFFFFF",
      fontface = "italic",
      family = "Avenir",
      size = 6,
  ...
  climb <- summary_data %>%
    ...
    geom_text(
      data = lbl_df,
      aes(x = yr, y = total_climb, label = climb_lbl),
      colour = "#FFFFFF",
      fontface = "italic",
      family = "Avenir",
      size = 6,
  ...
  sub_title_string <- str_glue(
    "{min(rider_index$start_date_local) %>% as_date() %>% format('%B %Y')} to {max(rider_index$start_date_local) %>% as_date() %>% format('%B %Y')}
  ...
  ggsave(
    filename = str_glue("coastal_vis_{rider}.png"),
    device = "png",
    width = a2_width,
    height = a2_height,
    units = "mm",
